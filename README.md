## 스프링 리액터 기본기
- [스프링 리액터 Reactor 기초 글 목록 by 최범균 ](https://javacan.tistory.com/entry/spring-reactor-intro-list)
- [flatMap과 map 비교](https://luvstudy.tistory.com/95)
- [[카카오][세미나] Webflux로 막힘없는 프로젝트 만들기](https://wedul.site/700?category=595982)
  - https://if.kakao.com/session/107
- [왜 리액티브인가](https://javacan.tistory.com/entry/why-reactive-summary?category=699082)
- [사용하면서 알게 된 Reactor, 예제 코드로 살펴보기 by kakao](https://tech.kakao.com/2018/05/29/reactor-programming/)
## 멀티 모듈 세팅 참고 링크
- https://github.com/Allali84/gradle-multiple-project
- https://github.com/znftm97/kotlin_springboot_multimodule_setting

## R2DBC 참고 링크
- https://github.com/kakaohairshop/spring-r2dbc-study
- https://github.com/hantsy/spring-r2dbc-sample
- https://github.com/spring-projects/spring-data-examples/tree/main/r2dbc
- https://r2dbc.io/spec/0.8.1.RELEASE/spec/html/#datatypes.mapping

## 연관관계 구현, CustomRepository
- https://heesutory.tistory.com/34?category=901813
- https://www.sipios.com/blog-tech/handle-the-new-r2dbc-specification-in-java

## kotlin에서 @Transient 안먹는 이슈
- https://github.com/spring-projects/spring-data-r2dbc/issues/449